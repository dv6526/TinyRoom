  ...
  function dropdownButtonClick(option) {
    ...
    switch (option.target.innerHTML) {
      ...
      case "mute":
        ...
        chat.mutedPlayers.push(dropdown_info.username);
        break;
      case "unmute":
        chat.socket.send("UN " + JSON.stringify({
          "username": dropdown_info.username
        }));
        chat.mutedPlayers = chat.mutedPlayers.filter(player => player !== dropdown_info.username);
        break;
      case "global mute":
        chat.socket.send("GM " + JSON.stringify({
          "username": dropdown_info.username
        }));
        chat.globalMutes.push(dropdown_info.username);
        break;
      case "global unmute":
        chat.socket.send("GU " + JSON.stringify({
          "username": dropdown_info.username
        }));
        chat.globalMutes = chat.globalMutes.filter(player => player !== dropdown_info.username);
        break;
      case "enter room":
  ...
  */
  ...
  for (let i = 0; i < optionsLength; i++) {
    ...
    if (i == 0)
      ...
      link.appendChild(document.createTextNode(options[i][0]));
      link.appendChild(document.createTextNode(options[i][1]));
    }
...
export class Chat {
  ...
  room: any;
  private_message: any;
  private_message_recepient: any;
  furniture: any;
  assets: any;
  background_clear_color: any;
  background_width: any;
  background_height: any;
  background_size: any;
  background_center: any;
  offset: any;
  dropdown_active: any;
  searchField_active: any;
  background: any;
  searchField: any;

  constructor(canvas_id, private cookieService: CookieService) {
          ...
          $.ajax({
            url: "api/uporabniki/" + found_user + "/profile", success: function (result) {
              ...
              messageDropdown({ x: click.clientX, y: click.clientY }, {
                'rank': this.cookieService.get('rank'),
                'muted': isMuted,
